...
def compile_shader2(vs, fs, tcs, tes):
    vert_shader = load_shader(vs)
    frag_shader = load_shader(fs)
    control_shader = load_shader(tcs)
    evaluation_shader = load_shader(tes)


    shader = OpenGL.GL.shaders.compileProgram(OpenGL.GL.shaders.compileShader(vert_shader, GL_VERTEX_SHADER),
                                              OpenGL.GL.shaders.compileShader(frag_shader, GL_FRAGMENT_SHADER),
                                              OpenGL.GL.shaders.compileShader(control_shader, GL_TESS_CONTROL_SHADER),
                                              OpenGL.GL.shaders.compileShader(evaluation_shader, GL_TESS_EVALUATION_SHADER))
    return shader
...
class ShaderProgram:
    ...
    def __init__(self, vs, fs, tcs, tes):
        self.shader = compile_shader2(vs, fs, tcs, tes)
